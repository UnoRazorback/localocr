...
private actor FakeOCRService: LocalOCRServing {
    ...
    private var failure: FakeFailure?

    func setFailure(_ failure: FakeFailure?) {
    ...
    func pageCount(at _: URL) async throws -> PageCountResponse {
        PageCountResponse(pages: 3)
    }

    func inspectPDF(at fileURL: URL) async throws -> InspectPDFResponse {
        InspectPDFResponse(sourcePath: fileURL.path, sourceSHA256: "inspect", pages: 1, searchablePages: 1, ocrNeededPages: 0, characters: 10, fullySearchable: true, pageDetails: [PageInspectionResponse(page: 1, characters: 10, searchable: true)])
    }
    ...
    func ocrImage(_: ImageOCRRequest) async throws -> ImageOCRResponse {
        ImageOCRResponse(text: "image text")
    }
