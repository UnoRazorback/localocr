...
@Suite struct IntelligenceChunkerTests {
    @Test func chunksKeepWholePagesAndTheirOriginalAttributionWhenTheyFit() throws {
        let document = IntelligenceDocument(pages: [
            .init(number: 2, text: "second page"),
            .init(number: 1, text: "first page")
        ])

        let chunks = try IntelligenceChunker.chunks(document: document, characterBudget: 11)
    ...
    @Test func chunksSplitOversizedPagesAtWordBoundariesWithoutDroppingText() throws {
        let document = IntelligenceDocument(pages: [.init(number: 4, text: "alpha beta gamma")])

        let chunks = try IntelligenceChunker.chunks(document: document, characterBudget: 10)
    ...
    @Test func chunksPreserveParagraphDelimitersWhenSplitting() throws {
    ...
    @Test func chunksPreserveStablePageAndFragmentOrderAcrossPageBoundaries() throws {
        let document = IntelligenceDocument(pages: [
            .init(number: 3, text: "one two three"),
            .init(number: 1, text: "four five")
        ])

        let chunks = try IntelligenceChunker.chunks(document: document, characterBudget: 9)
    ...
    @Test func chunksRejectsZeroCharacterBudget() {
        let document = IntelligenceDocument(pages: [.init(number: 1, text: "invoice")])

        #expect(throws: IntelligenceError.contextOverflow) {
            try IntelligenceChunker.chunks(document: document, characterBudget: 0)
        }
    }

    @Test func chunksRejectsNegativeCharacterBudget() {
        let document = IntelligenceDocument(pages: [.init(number: 1, text: "invoice")])

        #expect(throws: IntelligenceError.contextOverflow) {
            try IntelligenceChunker.chunks(document: document, characterBudget: -1)
        }
    }
}
