...
@Suite struct IntelligenceChunkerTests {
    @Test func publicChunksCallDoesNotRequireTryAndKeepsWholePagesWhenTheyFit() {
    ...
    @Test func validatedChunksRejectsZeroCharacterBudget() {
        let document = IntelligenceDocument(pages: [.init(number: 1, text: "invoice")])

        #expect(throws: IntelligenceError.contextOverflow) {
            try IntelligenceChunker.validatedChunks(document: document, characterBudget: 0)
        }
    }

    @Test func validatedChunksRejectsNegativeCharacterBudget() {
        let document = IntelligenceDocument(pages: [.init(number: 1, text: "invoice")])

        #expect(throws: IntelligenceError.contextOverflow) {
            try IntelligenceChunker.validatedChunks(document: document, characterBudget: -1)
        }
    }
}
