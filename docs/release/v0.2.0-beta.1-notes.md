...
Built with stable Xcode 26.6 (`17F113`), Swift 6, and arm64, with a macOS 14 deployment target. This Developer ID-signed Hardened Runtime build is Apple notarized, stapled, and Gatekeeper accepted. Complete downloaded-package and GUI acceptance passed on Apple M5 and Apple M4 Macs tested on macOS 27 beta build `26A5388g`. Later Apple beta builds may introduce regressions; report the exact macOS version and build number. For support triage, say that later Apple beta builds may introduce regressions and include that exact version and build number.
